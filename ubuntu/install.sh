...
echo "=== CommanderBiz Ubuntu Installer ==="

# 1. Dependency Check
echo "[*] Checking dependencies..."
pkg update -y > /dev/null 2>&1
for pkg in proot tar wget ubuntu-archive-keyring; do # Changed debian-archive-keyring to ubuntu-archive-keyring
...
echo "[*] Downloading Ubuntu Image (This may take a while)..."
...
cat <<EOF > start-ubuntu.sh # Changed start-kali.sh to start-ubuntu.sh
#!/data/data/com.termux/files/usr/bin/bash
unset LD_PRELOAD
